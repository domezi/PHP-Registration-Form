...
',(support("comment")?"<td id='label-comment'$vb>".'Kommentar':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.7.6")."' alt='+' title='".'HinzufÃ¼gen'."'>".script("row_count = ".count($p).";"),'</thead>
<tbody>
...
';if(DB!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'>".confirm(sprintf('Drop %s?',DB))."\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.7.6")."' alt='+' title='".'HinzufÃ¼gen'."'>\n";echo'<input type="hidden" name="token" value="',$qi,'">
</form>
